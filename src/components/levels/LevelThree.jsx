...
export function LevelThree() {
  ...
  return (
    <motion.section
      className="overflow-y-auto overflow-x-hidden max-h-[280vh] grid place-items-center h-[100vh]"
      initial={{ y: -1000 }}
      animate={{ y: 0 }}
      exit={{ y: -1000 }}
    ...
    >
      ...
      <div onClick={checkIfWin} className="level three">
        <AnimatePresence>
      ...
      <dialog
        open={openModal}
        className="h-36 w-72 rounded-lg bg-color items-center justify-center gap-3"
      >
        <input
          onChange={handleNameChange}
          className="ml-8 mt-6 mb-4 py-2 px-2 w-56 rounded-sm h-8 text-xl"
          placeholder="Name"
        />
        <motion.button
          ...
          className="ml-8 game-button w-56 h-12 text-lg rounded-lg"
        >
          Submit
        </motion.button>
      </dialog>
